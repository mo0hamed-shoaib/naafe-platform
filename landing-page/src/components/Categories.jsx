function Categories() {
  ...
  const defaultImage = "https://via.placeholder.com/300x300.png?text=لا يوجد صورة"; // صورة افتراضية إذا فشل التحميل

  return (
    <section className="py-16 sm:py-24 bg-naafe-cream" id="categories">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-naafe-teal">الفئات المميزة</h2>
          <p className="text-lg text-gray-600 mt-2">استكشف الخدمات الشائعة.</p>
        </div>
          ...
          {categories.map((category, index) => (
            <div key={index} className="card bg-white shadow-lg hover:shadow-xl transition-all duration-300 rounded-lg overflow-hidden">
              ...
              <div className="card-body items-center text-center p-4">
                <p className="text-gray-800 text-base md:text-lg font-semibold">{category.name}</p>
              </div>
            </div>
          ))}
        </div>
